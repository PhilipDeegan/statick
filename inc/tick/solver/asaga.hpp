...
namespace asaga {
namespace sparse {
template <bool INTERCEPT, typename HISTORY, typename T, typename Sparse2D, typename PROX,
          typename NEXT_I>
auto threaded_solve(const Sparse2D &features, const T *labels, std::atomic<T> *gradients_average,
                    std::atomic<T> *gradients_memory, T *iterate, T *steps_correction,
                    PROX call_single, NEXT_I _next_i, size_t n_threads, size_t n_thread,
                    HISTORY &history) {
  double step = 0.00257480411965l;
  const size_t n_samples = features.rows(), n_features = features.cols();
  size_t n_epochs = n_samples, epoch_size = n_samples;
...
template <bool INTERCEPT = false, typename T, typename HISTORY = tick::solver::History<T>,
          typename Sparse2D, typename PROX, typename NEXT_I>
void solve(const Sparse2D &features, const T *labels, std::atomic<T> *gradients_average,
              std::atomic<T> *gradients_memory, T *iterate, T *steps_correction, PROX call_single,
              NEXT_I _next_i, size_t n_threads, HISTORY &&history = HISTORY()) {
  std::vector<std::thread> threads;
  for (size_t i = 0; i < n_threads; i++) {
    threads.emplace_back([&]() {
      threaded_solve<INTERCEPT>(features, labels, gradients_average, gradients_memory, iterate,
                                steps_correction, call_single, _next_i, n_threads, i, history);
    });
  }
  for (size_t i = 0; i < n_threads; i++) {
    threads[i].join();

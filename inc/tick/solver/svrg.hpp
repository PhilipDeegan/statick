...
void solve_thread(SVRG_DAO &dao, typename MODEL::DAO &modao, PROX call, T *iterate, NEXT_I fn_next_i, size_t &t,
  ...
  for (size_t k = 0; k < (epoch_size / n_threads); k++) {
    ...
    for (size_t j = 0; j < iterate_size; ++j)
      iterate[j] = iterate[j] - dao.step * (dao.grad_i[j] - dao.grad_i_fixed_w[j] + dao.full_gradient[j]);
    ...
    for (size_t i = 0; i < n_threads; i++)
      threadsV.emplace_back([&]() {
        for (size_t j = 0; j < (epoch_size / n_threads); ++j)
          solve_thread<MODEL, RM, ST, INTERCEPT>(dao, modao, call, iterate, fn_next_i, t, n_threads, epoch_size);
      });
...
template <uint16_t RM = VarianceReductionMethod::Last, uint16_t ST = StepType::Fixed, bool INTERCEPT = false,
          typename MODEL, typename PROX, typename T, typename Sparse2D, typename DAO, typename NEXT_I>
void solve_thread(const Sparse2D &features, const T *labels, T *iterate, T *steps_correction, DAO &&dao,
...
template <uint16_t RM = VarianceReductionMethod::Last, uint16_t ST = StepType::Fixed, bool INTERCEPT = false,
          typename MODEL, typename PROX, typename T, typename Sparse2D, typename DAO, typename NEXT_I>
void solve(const Sparse2D &features, const T *labels, T *iterate, T *steps_correction, DAO &dao, NEXT_I fn_next_i,
           size_t &t, size_t n_threads, size_t epoch_size) {
  double step = 0.00257480411965l;
    ...
    for (size_t i = 0; i < n_threads; i++)
      threadsV.emplace_back([&]() {
        for (size_t t = 0; t < (epoch_size / n_threads); ++t)
          solve_thread<RM, ST, INTERCEPT, MODEL, PROX>(features, labels, iterate, steps_correction, dao, fn_next_i, t,
                                                       n_threads, epoch_size);
      });
    for (size_t i = 0; i < n_threads; i++) threadsV[i].join();
  } else {
    for (size_t i = 0; i < epoch_size; ++i)

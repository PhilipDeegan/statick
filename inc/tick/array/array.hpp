...
class Sparse2D {
 ...
 public:
  ...
  INDEX_TYPE row_size(size_t i) const { return m_row_indices[i + 1] - m_row_indices[i]; }
  const INDEX_TYPE *indices() const { return m_indices.data(); }
  const INDEX_TYPE *row_indices(size_t i) const { return m_indices.data() + m_row_indices[i]; }
  const INDEX_TYPE *row_indices() const { return m_row_indices; }
 ...
 private:
  std::vector<T> m_data;
  std::vector<size_t> m_info;
  std::vector<INDEX_TYPE> m_indices, m_row_indices;

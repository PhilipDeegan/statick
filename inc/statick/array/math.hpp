...
T sum(const T *x, const size_t size) {
  T t{0};
...
template <typename T> static inline
void copy(const T *from, T *to, const size_t size) {
...
template <typename T> static inline
T dot(const T *const x, const T *const y, const size_t size) {
  return kul::math::dot(size, x, y);
}

template <typename T, typename Y> static inline
...
T max(const T *x, size_t size) {
  T _m{0};
  for (size_t i = 0; i < size; i++) if (x[i] > _m) _m = x[i];
  return _m;
}
...
template <typename T> static inline
T norm_sq(const T *array, const size_t size) {
  T n_sq{0};
...
template <typename T> static inline
T abs(const T f) { return f < 0 ? f * -1 : f; }

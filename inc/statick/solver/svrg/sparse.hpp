...
template <typename MODEL, uint16_t RM, uint16_t ST, bool INTERCEPT, typename PROX, typename DAO,
...
void solve_thread(DAO &dao, typename MODEL::DAO &modao, PROX prox, size_t n_thread) {
  ...
  for (size_t t = 0; t < thread_epoch_size; ++t) {
    ...
    T grad_i_diff = MODEL::grad_i_factor(modao, iterate, i) -
                    MODEL::grad_i_factor(modao, dao.fixed_w.data(), i);
    for (size_t idx_nnz = 0; idx_nnz < x_i_size; ++idx_nnz) {
      ...
      T descent_direction = step * (x_i[idx_nnz] * grad_i_diff + step_correction * full_gradient_j);
    ...
    if constexpr (INTERCEPT) {
      ...
      iterate[n_features] = PROX::call_single(
          prox,
          iterate[n_features] -
              (/*descent_direction =*/step * (grad_i_diff + full_gradient[n_features])),
          step);
    }
    ...
    if constexpr (RM == VarianceReductionMethod::Random)
      if (t == dao.rand_index) dao.next_iterate = iterate;
    if constexpr (RM == VarianceReductionMethod::Average)
      dao.next_iterate.mult_incr(iterate, 1.0 / epoch_size);
  }
}

template <typename MODEL, uint16_t RM = VarianceReductionMethod::Last,
          uint16_t ST = StepType::Fixed, bool INTERCEPT = false, typename PROX, typename DAO>
void solve(DAO &dao, typename MODEL::DAO &modao, PROX &prox) {
  ...
  auto log_history = [&](size_t epoch) {
    ...
    if ((last_record_epoch + epoch) == 1 ||
        ((last_record_epoch + epoch) % log_every_n_epochs == 0)) {
  ...
  for (size_t i = 1; i < n_threads - 1; i++)
    funcs.emplace_back([&]() { solve_thread<MODEL, RM, ST, INTERCEPT>(dao, modao, prox, i); });
  ...
  for (size_t epoch = 1; epoch < (n_epochs + 1); ++epoch) {
    ...
    if constexpr (RM == VarianceReductionMethod::Last)
      for (size_t i = 0; i < dao.iterate.size(); i++) dao.next_iterate[i] = iterate[i];
  }
  dao.t += epoch_size;

...
namespace detail {
inline PyCSR_Proxy* csr_proxy(void* ptr) {
    return reinterpret_cast<PyCSR_Proxy*>(ptr);
}
...
template <typename T> struct csr_info_scalar {
    typedef T type;
...
template <typename T> struct csr_info : csr_info_scalar<T> { };
...
public:
    ...
    csr(const pybind11::dtype &dt, ShapeContainer shape, const void *ptr = nullptr, handle base = handle())
    ...
    ssize_t ndim() const {
        return detail::csr_proxy(m_ptr)->nd;
    }

    /// Base object
    object base() const {
        return reinterpret_borrow<object>(detail::csr_proxy(m_ptr)->base);
    }

    /// Dimensions of the csr
    const ssize_t* shape() const {
        return detail::csr_proxy(m_ptr)->dimensions;
    }
    ...
    const ssize_t* strides() const {
        return detail::csr_proxy(m_ptr)->strides;
    }
    ...
    int flags() const {
        return detail::csr_proxy(m_ptr)->flags;
    }
    ...
    void resize(ShapeContainer new_shape, bool refcheck = true) {
        ...
        if (isinstance<csr>(new_csr)) { *this = std::move(new_csr); }
    }
...
protected:
    ...
    void check_writeable() const {
        if (!writeable())
            throw std::domain_error("csr is not writeable");
    }

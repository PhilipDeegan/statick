...
void sparse(int argc, char *argv[]) {
  ...
  auto dev_lambda = [&](size_t dev_id) {
    try {
      ...
      size_t features_info[3], labels_info[1];
      {
        std::ifstream bin_data(features_s, std::ios::in | std::ios::binary);
        cereal::PortableBinaryInputArchive iarchive(bin_data);
        statick::hcc::load_sparse2d_info_only(iarchive, features_info);
      }
      {
        std::ifstream bin_data(labels_s, std::ios::in | std::ios::binary);
        cereal::PortableBinaryInputArchive iarchive(bin_data);
        statick::hcc::load_array_info_only(iarchive, labels_info);
      }
      std::vector<std::vector<double>> data, vlabels;
      std::vector<std::vector<size_t>> info;
      std::vector<std::vector<INDICE_TYPE>> indices, row_indices;

      std::vector<hc::array<double, 1>> hc_data, hc_labels;
      std::vector<hc::array<size_t, 1>> hc_info;
      std::vector<hc::array<INDICE_TYPE, 1>> hc_indices, hc_row_indices;

      std::vector<hc::array_view<double, 1>> ar_data, ar_labels;
      std::vector<hc::array_view<size_t, 1>> ar_info;
      std::vector<hc::array_view<INDICE_TYPE, 1>> ar_indices, ar_row_indices;

      for (size_t i = 0; i < TIMES; i++) {
        data.emplace_back(features_info[2]);
        info.emplace_back(5);
        indices.emplace_back(features_info[2]);
        row_indices.emplace_back(features_info[1] + 1);
        std::ifstream bin_data(features_s, std::ios::in | std::ios::binary);
        cereal::PortableBinaryInputArchive iarchive(bin_data);
        statick::hcc::load_raw_sparse2d_with_raw_data(iarchive, &data[i][0], &info[i][0],
                                                   &indices[i][0], &row_indices[i][0]);
        ar_data.emplace_back(features_info[2], data.back().data());
        hc_data.emplace_back(ar_data.back(), acc_views.back());
        ar_info.emplace_back(5, info.back().data());
        hc_info.emplace_back(ar_info.back(), acc_views.back());
        ar_indices.emplace_back(features_info[2], indices.back().data());
        hc_indices.emplace_back(ar_indices.back(), acc_views.back());
        ar_row_indices.emplace_back(features_info[1] + 1, row_indices.back().data());
        hc_row_indices.emplace_back(ar_row_indices.back(), acc_views.back());
      }
      RAM_HERE;
      ...
      std::vector<double> steps_corrections(
          statick::saga::sparse::compute_step_corrections(l_features));
      RAM_HERE;
      ...
      for (size_t i = 0; i < TIMES; i++) {
        vlabels.emplace_back(N_SAMPLES);
        std::ifstream data(labels_s, std::ios::in | std::ios::binary);
        cereal::PortableBinaryInputArchive iarchive(data);
        statick::hcc::load_array_with_raw_data(iarchive, &vlabels[i][0]);
        ar_labels.emplace_back(N_SAMPLES, vlabels.back().data());
        hc_labels.emplace_back(ar_labels.back(), acc_views.back());
      }
      RAM_HERE;

      std::vector<hc::array<double, 1>> gradients_average, gradients_memory, iterate;
      std::vector<hc::array<double, 1>> gradients_average, gradients_memory, iterate;
      std::vector<hc::array<double, 1>> gradients_average, gradients_memory, iterate;
      for (size_t i = 0; i < TIMES; i++) {
        gradients_average.emplace_back(N_FEATURES, acc_views.back());
        gradients_memory.emplace_back(N_SAMPLES, acc_views.back());
        iterate.emplace_back(N_FEATURES, acc_views.back());
      }
      ...
      //   hc::parallel_for_each(acc_views.back(), hc::extent<1>(1), [=](hc::index<1> i) [[hc]] {
      //     ar_gradients_average[0] = 10 + dev_id;
      // }));
      // for(auto f : futures) f.wait();
      // std::cout << __LINE__ << " : " << dev_id  << " : "  << ar_gradients_average[0] <<
      // std::endl;
      // futures.clear();
      // futures.emplace_back(
      ...
      //       statick::saga::sparse::solve(l_features[tim[i]],

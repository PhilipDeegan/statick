  ...
  for (auto acc = accelerators.begin(); acc != accelerators.end(); acc++) {
    for (int i = 0; i < numViewPerAcc; i++) {
      ...
      hc::completion_future cpfuture_x =
          hc::copy_async(host_x.begin() + dataCursor, host_x.begin() + newDataCursor, x_array);
      ...
      hc::completion_future cpfuture_y =
          hc::copy_async(host_y.begin() + dataCursor, host_y.begin() + newDataCursor, y_array);
      dataCursor = newDataCursor;
      cpfuture_x.wait();
      cpfuture_y.wait();

      hc::completion_future f;
      f = hc::parallel_for_each(
          acc_views.back(), x_array.get_extent(),
          [&, a ](hc::index<1> i)[[hc]] { y_array[i] = a * x_array[i] + y_array[i]; });
      futures.push_back(f);

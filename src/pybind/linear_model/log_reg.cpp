...
PYBIND11_MODULE(statick_linear_model, m) {
  ...
  py::class_<LOGREG_DAO_dd>(m, "log_reg_dao_dd").def(py::init<py_array_t<double> &, py_array_t<double> &>());
  py::class_<LOGREG_DAO_sd>(m, "log_reg_dao_sd").def(py::init<py_csr_t<double> &, py_array_t<double> &>());
  py::class_<LOGREG_DAO_ds>(m, "log_reg_dao_ds").def(py::init<py_array_t<float> &, py_array_t<float> &>());
  py::class_<LOGREG_DAO_ss>(m, "log_reg_dao_ss").def(py::init<py_csr_t<float> &, py_array_t<float> &>());
}
}

...
class PySolvers {
 public:
  static pybind11::class_<D> & INIT(pybind11::class_<D> &&m){return m.def(py::init<typename M::DAO &>()); }
};
...
auto def_solver(pybind11::module &m, pybind11::class_<D>&(*init)(pybind11::class_<D> &&) = &PySolvers<M, D>::INIT ){
  ...
  if constexpr (M::DAO::FEATURE::is_sparse)     st << "s"; else st << "d";
  if constexpr (std::is_same<double, T>::value) st << "d"; else st << "s";
...
auto def_multithreaded_solver(pybind11::module &m){
  ...
  statick::def_solver<S, M, D>(m, [](pybind11::class_<D>&& m) -> pybind11::class_<D>& {
    return m.def(py::init<typename M::DAO &, size_t /*n_epochs*/, size_t /*epoch_size*/, size_t /*threads*/>());

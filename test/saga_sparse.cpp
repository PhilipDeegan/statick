...
int main() {
  ...
  for (size_t j = 0; j < N_ITER; ++j) {
    tick::saga::sparse::solve<false>(*features.get(), labels->data(), gradients_average.data(), gradients_memory.data(),
                                     iterate.data(), steps_corrections.data(), call_single, next_i);

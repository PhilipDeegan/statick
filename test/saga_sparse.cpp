...
int main() {
  ...
  using namespace tick::saga::sparse;
  ...
  std::vector<double> iterate(n_features + static_cast<uint>(INTERCEPT)), objs,
      steps_corrections(compute_step_corrections(modao.features()));
  ...
  for (size_t j = 0; j < N_ITER; ++j) {
    solve<tick::TModelLogReg<double, DAO<double>>>(modao, iterate.data(), steps_corrections.data(), call_single,
                                                   next_i);

...
int main() {
  ...
  std::vector<double> iterate(N_FEATURES + static_cast<uint>(INTERCEPT)),
  ...
  for (size_t j = 0; j < N_ITER; ++j) {
    tick::asaga::sparse::solve(*features.get(), labels->data(), gradients_average.data(),
                               gradients_memory.data(), iterate.data(), steps_corrections.data(),
                               call_single, next_i, 12, history);
    if (j % 10 == 0)
